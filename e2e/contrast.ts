 ...
 *    files the node under `incomplete`. `body` here paints two radial
 *    `color-mix` washes over a vertical `linear-gradient`, `.exhibit` sits on
 *    another `linear-gradient`, and dozens of surfaces are
 *    `color-mix(in srgb, ... , transparent)`. Almost nothing on this page
 *    composites onto a flat colour.
 ...
 *    `opacity: .85` here, and the walkthrough's inactive bits at `0.6`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The vocabulary and
 *    detectability tables sit in `.table-wrap` scrollers, so at 380px much of
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This lab draws
 *    every figure into a `<canvas>` rather than SVG, so the rule is silent here
 *    — but it is kept because it is cheap: where an `<svg>` does
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: this stylesheet builds
     * nearly every surface with `color-mix(in srgb, ...)`. Chromium reports
     * those
     ...
     * radial washes anchored at 15%/-10% and 80%/0% that fade to transparent
     * by ~40%, so judging Exhibit 6 at the bottom of a very long page by the
     * corner colour would invent a backdrop nothing down there sits on. A non-gradient layer (`url()`, `none`) is unmeasurable and
     * paints nothing.
     */
     ...
     *
     * This is one very long single-page document — six exhibits, several
     * tables and a dozen canvas figures — and every text node walks the same
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has one native `<details>` — the
      // `.deep-dive` arms-race history in Exhibit 1 — and the gate scans both
      // its closed and its open configuration explicitly.
     ...
     * it, the `aria-hidden` decorations on this page — the `.exhibit-step`
     * numerals, the walkthrough's bit-value legend and the "↗" glyphs on the
     * outbound links — are measured as if they took the element's declared
     * `color`, which describes nothing on screen.
     */
     ...
     * lab authors no SVG at all — every figure is a `<canvas>` — so the rule is
     * silent here.
     */
